export function buildSamplePdf(lines: readonly string[] = ['Hello PDF.js'], pageCount = 1): Uint8Array {
  ...
  const pagesStart = 3;
  const contentsStart = pagesStart + pageCount;
  const fontId = contentsStart + pageCount;

  for (let index = 0; index < pageCount; index += 1) {
    const pageId = pagesStart + index;
    const contentId = contentsStart + index;
    kids.push(`${pageId} 0 R`);
    const pageLines = pageCount > 1 ? [`Page ${index + 1}`, ...lines] : lines;
